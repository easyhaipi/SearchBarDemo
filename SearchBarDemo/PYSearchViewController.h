...
typedef void(^PYDidSearchBlock)(PYSearchViewController *searchViewController, CYSearchBar *searchBar, NSString *searchText); // 开始搜索时调用的block
...
typedef NS_ENUM(NSInteger, PYSearchHistoryStyle) {  // 搜索历史风格
    PYSearchHistoryStyleCell = 1,           // UITableViewCell 风格
...
@property (nonatomic, strong) NSArray * optionsArray; //选项数据数组
...
@property (nonatomic, strong) CYSearchBar * searchBar;
